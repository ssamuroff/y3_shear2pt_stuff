...
class plot2D(plots.MultinestPlots2D):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.colors=['darkmagenta', 'hotpink', 'k','purple','royalblue', 'k','pink','steelblue',"purple", 'k', 'k', 'k', 'k']*10
        self.linestyles=['-','-','--',':']*10
        pylab.style.use('y1a1')
        #matplotlib.rcParams['text.usetex']=False
        matplotlib.rcParams['font.family']='serif'
        self.labels= [r'$\gamma \gamma$, TATT without B-modes',
                      r'$\gamma \gamma$, TATT with B-modes',
                      r'$\gamma \gamma + $\delta_g \gamma + \delta_g\delta_g$, TATT + $z$ power law',
                      r'$\gamma \gamma$, NLA',
        ...
        self.plot_points=[] #(0.316, 0.808471776802354*np.sqrt(0.316/0.3))]
        #self.proxies=[]
    ...
    def make_2d_plot(self, name1, name2):
        ...
        if self.labels is not None:
            leg=pylab.legend(self.proxies,self.labels,loc="upper left", fontsize=13)
        ...
        pylab.xlabel(r"$A_1$",fontsize=22, fontname='serif')
