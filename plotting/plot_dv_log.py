...
def cornerplot(theory, data, show_cuts=False):
    ...
    for i in range(ni):
        for j in range(nj):
            ...
            ax.annotate("(%d, %d)"%(i+1,j+1), (3.,2.5e-7), textcoords='data', fontsize=9, )
            ...
            if (posp==19) or (posp==1) or (posp==7) or (posp==13):
                ...
                plt.yticks([1e-7,1e-6,1e-5,1e-4],fontsize=ytickfontsize)
            else:
                plt.yticks(visible=False)

            if (posp==19):
                plt.ylabel(r"$ \theta \xi_+ $", fontsize=11)
           ...
           # xip_b_remapped = (interp_xip_b(np.log10(xip_b[0])))
            ...
            plt.errorbar(xp, xp*xip, yerr=xp*dxip, marker='.', linestyle='none', markerfacecolor='k', markeredgecolor='k', ecolor='k',markersize=markersize)
            p1 = plt.plot(xta,xta*xip_theory,color='darkmagenta',lw=1.5, label='GG+GI+II')
            p2 = plt.plot(xta,xta*xip_theory_gi,color='pink',lw=1.5, ls='-', label='GI')
            plt.plot(xta,-xta*xip_theory_gi,color='pink',lw=1.5, ls='--')
            p3 = plt.plot(xta,xta*xip_theory_ii,color='midnightblue',lw=1.5, ls=':', label='II')
            plt.plot(xta,xta*(xip_theory_ii+xip_theory_gi),color='steelblue',lw=1., ls='-')
            plt.plot(xta,-xta*(xip_theory_ii+xip_theory_gi),color='steelblue',lw=1., ls='--')
            ...
            ax.annotate("(%d, %d)"%(i+1,j+1), (3,1.5e-4), textcoords='data', fontsize=9, )
            ax.yaxis.set_tick_params(which='minor', left='off', right='off')
...
#            ax.xaxis.set_minor_locator(MultipleLocator(10))
            ...
            plt.errorbar(xm, xm*xim, yerr=xm*dxim, marker='.', linestyle='none', markerfacecolor='k', markeredgecolor='k', ecolor='k',markersize=markersize)
            plt.plot(xta,xta*xim_theory,color='darkmagenta',lw=1.5)
            plt.plot(xta,xta*xim_theory_gi,color='pink',lw=1.5, ls='-')
            plt.plot(xta,-xta*xim_theory_gi,color='pink',lw=1.5, ls='--')
            plt.plot(xta,xta*xim_theory_ii,color='midnightblue',lw=1.5, ls=':')
            plt.plot(xta,xta*(xim_theory_ii+xim_theory_gi),color='steelblue',lw=1., ls='-')
            plt.plot(xta,-xta*(xim_theory_ii+xim_theory_gi),color='steelblue',lw=1., ls='--')
...
#    ax = plt.subplot(111)
    ...
    plt.legend(proxies,["GG+GI+II", "GI+II", "GI", 'II'], loc='upper right', bbox_to_anchor=(legend_x, legend_y), fontsize=9)

    plt.subplots_adjust(hspace=0,wspace=0, bottom=0.16,left=0.16, right=0.88)
    plt.savefig("plots/unblinded_datavector_xipm_log.pdf")
    plt.savefig("plots/unblinded_datavector_xipm_log.png")
