...
def cornerplot(theory1, theory2, data1, data2, show_cuts=False):
    ...
    for i in range(ni):
        for j in range(nj):
            ...
            if (posp==19):
                plt.ylabel(r"$\Delta \xi_+(\theta)/\xi_+$", fontsize=10)
                plt.xlabel(r"$\theta$ / arcmin", fontsize=10)
                plt.yticks([-0.04,-0.02,0,0.02,0.04],fontsize=8)
            else:
                pass
            ...
            ax.annotate("(%d, %d)"%(i+1,j+1), (37,0.03), textcoords='data', fontsize=9, )
            ax.yaxis.set_tick_params(which='minor', left='off', right='off')
            plt.ylim(-ymin,ymin)
            ...
            for iline,(t1, t2) in enumerate(zip(theory1,theory2)):
                ...
                interp_xim_a = sint.interp1d(np.log10(xta), xim_theory_a, kind='cubic', bounds_error=None, fill_value='extrapolate')
                interp_xim_b = sint.interp1d(np.log10(xtb), xim_theory_b, kind='cubic', bounds_error=None, fill_value='extrapolate')
    ...
    plt.savefig("bmodes_diff.pdf")
    plt.savefig("bmodes_diff.png")
