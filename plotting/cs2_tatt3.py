...
samp1 = np.array([c1.samples['cosmological_parameters--omega_m'], c1.samples['cosmological_parameters--s8'], c1.samples['intrinsic_alignment_parameters--a1'], c1.samples['intrinsic_alignment_parameters--a2']]) #, c1.samples['intrinsic_alignment_parameters--a1']*c1.samples['intrinsic_alignment_parameters--bias_ta']])
...
samp5 = np.array([c5.samples['cosmological_parameters--omega_m'], c5.samples['cosmological_parameters--s8'], c5.samples['intrinsic_alignment_parameters--a1'], c5.samples['intrinsic_alignment_parameters--a2']]) #, c5.samples['intrinsic_alignment_parameters--a1']*c5.samples['intrinsic_alignment_parameters--bias_ta']])
...
#3775A1
#DD9EE8
colours = ['#7223AD','#000000','#A4CD64','#3775A1','#A4CD64','#3775A1']
...
samples2 = MCSamples(samples=samp2.T,names=['x1','x2','x3'], labels=names[:-2], label='NLA', weights=c2.weight, settings={'boundary_correction_order':0, 'mult_bias_correction_order':1})
samples3 = MCSamples(samples=samp3.T,names=['x1','x2','x3'], labels=names[:-2], label='NLA (no $z$)', weights=c3.weight, settings={'boundary_correction_order':0, 'mult_bias_correction_order':1})
samples4 = MCSamples(samples=samp4.T,names=['x1','x2'], labels=names[:-3], label='No IAs', weights=c4.weight, settings={'boundary_correction_order':0, 'mult_bias_correction_order':1})
samples5 = MCSamples(samples=samp5.T,names=['x1','x2','x3','x4'], labels=names, label='TATT optimized', weights=c5.weight, settings={'boundary_correction_order':0, 'mult_bias_correction_order':1})

g.triangle_plot([samples, samples5, samples2,samples3],['x1','x2','x3','x4'], filled=[True,False,False,False,False,True], contour_args=[{'alpha':0.6},{'alpha':1.,'ls':'--'},{'alpha':1},{'alpha':1.}], diag1d_kwargs={'normalized':True}, contour_colors=colours, labels=['TATT', 'NLA', 'NLA (no $z$)'], param_limits={'x1':(0.18,0.51), 'x2':(0.69,0.85), 'x3':(-1.8,2),'x4':(-2.5,4.5)}) #, markers=[[0.15,0.3,0.45], [0.6,0.7,0.8,0.9], [-2,-1,0,1,2,3]])
#import pdb ; pdb.set_trace()
...
plt.savefig('/Users/hattifattener/Documents/y3cosmicshear/plots/cs2/unblinded_y3_1x2pt_tatt_v3_getdist_noa1delta.pdf')
plt.savefig('/Users/hattifattener/Documents/y3cosmicshear/plots/cs2/unblinded_y3_1x2pt_tatt_v3_getdist_noa1delta.png')
