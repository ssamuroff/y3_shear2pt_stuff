...
def cornerplot(theory, data, show_cuts=False):
    ...
    for i in range(nj):
        for j in range(ni):
            ...
            ax.annotate("(%d, %d)"%(j+1,i+1), (3.8,1.2), textcoords='data', fontsize=11, )
            ...
            if j in [0,6,11,17]:
                plt.ylabel(r"$\theta \gamma_t\times 10^{2}$", fontsize=11)
            ...
            plt.errorbar(xa, 1e2 * xa*gammat_a, yerr=1e2*xa*dgammat_a, ls="none", marker=".", markersize=2,  ecolor="k", markeredgecolor="k", markerfacecolor="k")
            

            plt.plot(xta, 1e2*xta*gammat_theory_a,color="darkmagenta", ls='-',lw=1.5)
            plt.plot(xta, 1e2*xta*gammat_theory_gi,color="pink", ls='--',lw=1.5)
    ...
    plt.savefig("plots/unblinded_datavector_gammat.pdf")
    plt.savefig("plots/unblinded_datavector_gammat.png")
